...
public class ServerApplication {
    ...
    public static void main(String[] args) {
        ...
        // 💡 1. [변경] PDF 건물이 모인 최상위 루트 폴더 경로 및 결과 JSON 파일들이 저장될 디렉토리 경로 지정
        String buildingRootFolderPath = "src/main/resources/building"; // 이 폴더 밑에 '정보공학관', '인문관' 등의 폴더가 위치함
        String jsonOutputDirPath = "src/main/resources/masterfile";       // '정보공학관.json', '인문관.json' 등이 저장될 폴더

        // 💡 2. [변경] 여러 건물의 폴더를 순회하며 건물별 마스터 시간표 JSON 템플릿들을 생성/업데이트
        System.out.println("🔄 각 건물별 PDF 파일로부터 마스터 시간표 데이터를 동기화하는 중...");
        ScheduleInitializer initializer = new ScheduleInitializer();
        
        // 새로 확장한 메서드로 교체 호출합니다.
        initializer.convertAllBuildingsToJsons(buildingRootFolderPath, jsonOutputDirPath);
        
        // 💡 3. [변경] 서비스 로직 초기화 (단일 파일 경로가 아닌 JSON들이 모여있는 폴더 디렉토리 경로를 주입)
