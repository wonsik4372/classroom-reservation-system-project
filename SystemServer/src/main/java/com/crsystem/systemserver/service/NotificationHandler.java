...
public class NotificationHandler {
    ...
    public void notifyApproved(ReservationDTO.Response reservation) {
        ...
        NotificationCatalog.getInstance().addNotification(notification);
    }
    ...
    public ResponseDTO getAndMarkNotifications(String userId) {
        List<NotificationDTO> unread = NotificationCatalog.getInstance().getUnreadNotifications(userId);

        if (!unread.isEmpty()) {
            List<String> ids = unread.stream()
                    .map(NotificationDTO::getNotificationId)
                    .collect(java.util.stream.Collectors.toList());
            NotificationCatalog.getInstance().markAsRead(userId, ids);
        }
